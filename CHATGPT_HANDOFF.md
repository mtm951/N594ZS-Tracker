...
- User confirmed that completed order #18374415 should remain searchable history, not clutter the active Orders queue. This is a presentation-only change to the ORIGINAL \x60app-05-views.js\x60. The Orders view opens with \x60Open — active orders\x60 selected. Other views: \x60History — received / cancelled\x60, \x60All orders\x60, and the original individual statuses. The chosen filter persists across Orders page rerenders within the same open app session; on next full load it defaults to Open again.
- A line is shown in Open if \x60!isClosedOrder(o)\x60 and in History if \x60isClosedOrder(o)\x60. Existing order records, receipt events, inventory and total spending are UNCHANGED; the History tab is an inclusive status filter, **not deletion, archival, or a second copy**. Search, project, system and sortable columns still operate with the selected filter. Received and Cancelled lines from a group remain separately inspectable in History; if some order lines are active, only those active lines appear in Open.
- Corrected the order group banner's \x60units remaining\x60 calculation to exclude closed lines, especially cancelled lines that otherwise misleadingly reported unreceived quantities as still due. This is display-only and does not adjust actual ordered, received or inventory quantities.
- Pre-change GitHub recovery branch \x60validated-v5.19.29-before-order-history-2026-09-24\x60 points to main \x6053eea7a3b2d2263d720fd7fb1c869ddd26b978e4\x60. No production cloud SQL writes or active aircraft data changes were required. \x60tests/order-multi-project-links.test.mjs\x60 now checks default Open, History/All/individual statuses, status persistence during rerender, retention of completed and cancelled group lines, no filter data mutations, and that receiving the remaining test LEDs moves the line from Open to History with only one inventory credit. Full regression workflow also runs on \x60orders-*\x60 branches. Phone and desktop visual confirmation are still owner acceptance tasks.
