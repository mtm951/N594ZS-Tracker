...
function openPartDetail(id){
  ...
  openModal(`${modalHeader(p.name,p.partNo?`PN / Spec: ${p.partNo}`:p.system)}
    ...
    <div class="detail-grid"><div>
      ...
      <div class="detail-card"><div class="section-tools"><h3>Orders</h3><button class="icon-btn" onclick="openOrderModal(null,null,${id})">+ Order</button></div>${orders.length?orders.map(o=>`<div class="kv click-row" onclick="openOrderDetail(${o.id})"><div><b>${esc(o.item)}</b><div class="task-note">${esc(o.vendor||'')} ${o.eta?'• ETA '+esc(o.eta):''}</div></div>${pill(o.status)}</div>`).join(''):'<div class="muted">No orders linked.</div>'}</div>
      <div class="detail-card"><h3>Linked Projects</h3>${linkedProjects.length?linkedProjects.map(pr=>`<div class="kv click-row" onclick="openProjectDetail(${pr.id})"><span>${esc(pr.title)}</span>${pill(pr.status)}</div>`).join(''):'<div class="muted">No projects linked.</div>'}</div>
