...
function injectPartSmartCards(partId){
  ...
  const html=`<div class="detail-card" id="smartPartInventory"><h3>Inventory Position</h3><div class="smart-status-strip"><div class="smart-status-card"><span>On hand</span><b>${on===null?'—':esc(on+' '+(p.unit||''))}</b></div><div class="smart-status-card"><span>Reserved</span><b>${esc(reserved+' '+(p.unit||''))}</b></div><div class="smart-status-card"><span>Free</span><b class="${free!==null&&free<0?'stock-short':'stock-ok'}">${free===null?'—':esc(free+' '+(p.unit||''))}</b></div><div class="smart-status-card"><span>Used in logs</span><b>${esc(partConsumedQty(p.id)+' '+(p.unit||''))}</b></div></div>${reservations.length?`<div class="detail-section"><label>Project reservations</label>${reservations.map(r=>`<div class="kv click-row" onclick="openProjectDetail(${r.project.id})"><span>${esc(r.project.title)}</span><b>${esc(r.item.qty)} ${esc(r.item.unit||p.unit||'ea')}</b></div>`).join('')}</div>`:''}<div class="detail-section"><label>Inventory movement / allocation history</label>${moves.map(m=>`<div class="movement-row"><span>${esc(m.date||'Planned')}</span><span class="movement-kind ${(m.kind==='IN'||m.kind==='RECEIPT')?'movement-in':m.kind==='OUT'?'movement-out':m.kind==='RESERVE'?'movement-reserve':''}">${esc(m.kind)}</span><span>${esc(m.desc)}</span><b>${m.kind==='OUT'?'-':m.kind==='IN'||m.kind==='RECEIPT'||(m.kind==='ADJUST'&&num(m.qty)>0)?'+':''}${esc(m.qty)} ${esc(m.unit)}</b></div>`).join('')||'<div class="muted">No linked movement history yet.</div>'}</div><div class="tiny muted" style="margin-top:8px">Movement history includes linked purchases, order receipts, manual adjustments, work-log consumption and project reservations. It describes saved activity; the On Hand figure remains the stock calculation.</div></div>`;
