...
function renderPurchases(){
  ...
  page.innerHTML=`<div class="card"><div class="toolbar"><div><h1>Purchases</h1><div class="muted">Purchase history, part-number memory, invoice files, inventory linkage and one-click reorder records.</div></div><div class="action-row"><button class="secondary" onclick="document.getElementById('purchaseImportFile').click()">Import Aircraft Spruce CSV</button><button class="primary" onclick="openPurchaseModal()">+ Add Purchase</button></div></div>
    <div class="purchase-metrics"><div><b>${s.lines}</b><span>Line items</span></div><div><b>${s.invoices}</b><span>Invoices</span></div><div><b>${s.pns}</b><span>Part numbers</span></div><div><b>${fmtMoney(s.total)}</b><span>Line-item subtotal</span></div><div><b>${s.unknown}</b><span>Disposition unknown</span></div><div><b>${fmtMoney(s.total)}</b><span>Item subtotal</span></div></div>
...
function purchaseMatches(p){const q=(val('purchaseSearch')||'').trim().toLowerCase(),d=val('purchaseDisposition'),sys=val('purchaseSystem');if(d&&p.disposition!==d)return false;if(!trackerRecordMatchesSystem(p,sys,'purchases'))return false;if(!q)return true;return `${p.pn} ${p.description} ${p.invoice} ${p.order} ${p.vendor} ${p.seller||''} ${p.system} ${p.notes}`.toLowerCase().includes(q)}
function renderPurchaseRows(){const box=document.getElementById('purchaseRows');if(!box)return;document.querySelectorAll('.segmented button').forEach((b,i)=>b.classList.toggle('active',(purchaseViewMode==='parts'?i===0:i===1)));
  if(purchaseViewMode==='parts'){
    const groups=purchaseGroups(db.purchases.filter(purchaseMatches)).sort((a,b)=>(b.lastDate||'').localeCompare(a.lastDate||''));
