...
async function openEquipmentDetail(id){
  ...
  const p=equipmentPurchase(e),part=e.inventoryPartId?partById(Number(e.inventoryPartId)):null,maint=db.maintenance.filter(m=>m.system===e.system),projects=e.linkedProjectIds.map(projectById).filter(Boolean);
