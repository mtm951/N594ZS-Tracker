...
let supa=null, cloudSession=null, cloudWorkspaceId=null, cloudSaveTimer=null, cloudLoading=false;

function cloudStatusLabel(text,kind=''){
  const el=document.getElementById('cloudStatus');
  if(!el)return;
  el.innerHTML=`<button class="secondary" onclick="${cloudSession?'openCloudAccount()':'openCloudAuth()'}">${esc(text)}</button>`;
}
...
async function initCloud(){
  try{
    ...
    supa.auth.onAuthStateChange(async(_event,session)=>{
      ...
      else {cloudWorkspaceId=null;cloudStatusLabel('Sign in to sync');}
    });
    if(cloudSession) await connectWorkspaceAndLoad();
  ...
  try{
    ...
    if(!membership){
      ...
      if(claim.error){
        ...
        toast('Signed in, but this account does not have access to N594ZS Tracker.','bad');
        return;
      }
      cloudWorkspaceId=claim.data;
    }else cloudWorkspaceId=membership.workspace_id;
...
function queueCloudSave(){
  if(!supa||!cloudSession||!cloudWorkspaceId||cloudLoading)return;
...
function openCloudAccount(){
  ...
  openModal(`${modalHeader('Cloud account',email)}<div class="card soft-card"><b>Shared sync is active</b><div class="muted small" style="margin-top:6px">Changes to projects, parts, orders, logs, documents, checklists, and aircraft data are saved to the shared N594ZS workspace.</div></div><div class="modal-actions"><button class="secondary" onclick="forceCloudReload()">Reload shared data</button><button class="danger" onclick="cloudSignOut()">Sign out</button></div>`,false);
}
