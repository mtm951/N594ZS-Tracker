...
function maintenanceRelatedProjects(m){
  const stop=new Set(['rotax','engine','maintenance','service','system','manufacturer','current','applicable','inspection','inspect','replace','replacement','component','source','manual','line','series','years','year','hours','hour','check','required','procedure']);
  const tokens=[m?.title,m?.system,m?.sourceNotes,m?.procedureSource].filter(Boolean).join(' ').toLowerCase().match(/[a-z0-9]+/g)||[];
  const wanted=[...new Set(tokens.filter(x=>x.length>=5&&!stop.has(x)))];
  return arr(db.projects).map(p=>{
    const stepText=arr(p.steps).map(s=>[s.text,s.note].filter(Boolean).join(' ')).join(' ');
    const updateText=arr(p.updates).map(u=>u.text||'').join(' ');
    const hay=[p.title,p.system,p.summary,p.plan,p.nextStep,p.blockers,p.completionCriteria,stepText,updateText].filter(Boolean).join(' ').toLowerCase();
    let score=wanted.reduce((n,t)=>n+(hay.includes(t)?1:0),0);
    if(String(p.system||'').toLowerCase()===String(m?.system||'').toLowerCase())score+=2;
    return {p,score};
  }).filter(x=>x.score>=2).sort((a,b)=>b.score-a.score||(a.p.status==='Done')-(b.p.status==='Done')).slice(0,3).map(x=>x.p);
}
function maintenanceRelatedWorkHTML(m){
  const projects=maintenanceRelatedProjects(m);if(!projects.length)return '';
  return `<div class="detail-card maintenance-related-card"><div class="section-tools"><div><h3>Related N594ZS Work & Notes</h3><div class="muted tiny">Notes already recorded elsewhere in the tracker that appear related to this maintenance item.</div></div></div>
    <div class="maintenance-related-list">${projects.map(p=>{
      const notes=arr(p.steps).filter(s=>String(s.note||'').trim()).map(s=>({label:s.text,note:s.note}));
      const updates=arr(p.updates).filter(u=>String(u.text||'').trim()).map(u=>({label:u.date||'Update',note:u.text}));
      const entries=[...notes,...updates].slice(0,6);
      return `<div class="maintenance-related-project"><button class="maintenance-related-head" onclick="openProjectDetail(${Number(p.id)})"><span><b>${esc(p.title)}</b><small>${esc(p.system||'General')} • ${esc(p.status||'')}</small></span><span class="maintenance-related-arrow">›</span></button>
        ${entries.length?`<div class="maintenance-related-notes">${entries.map(n=>`<div><span>${esc(n.label)}</span><b>${esc(n.note)}</b></div>`).join('')}</div>`:'<div class="muted tiny maintenance-related-empty">No step notes on this project.</div>'}
      </div>`;
    }).join('')}</div>
  </div>`;
}
function openMaintenanceDetail(id){
  ...
  const checklist=p.linked
    ?`<div class="detail-card maintenance-checklist-card"><div class="maintenance-checklist-head"><div><div class="maintenance-eyebrow">SOURCE CHECKLIST</div><h3>Procedure Checklist</h3><div class="muted tiny">${esc(p.linked.name)}</div></div><div class="maintenance-progress-count"><b>${p.done}</b><span>/ ${p.items.length}</span><small>${p.pct}% complete</small></div></div><div class="maintenance-progress-track"><i style="width:${p.pct}%"></i></div><div class="maintenance-procedure-preview">${p.items.slice(0,6).map((i,n)=>`<div class="${i.done?'done':''}"><span class="maintenance-step-number">${i.done?'✓':n+1}</span><span>${esc(i.text)}</span></div>`).join('')}${p.items.length>6?`<div class="maintenance-preview-more">+${p.items.length-6} more steps</div>`:''}</div><div class="maintenance-checklist-actions"><button class="primary" onclick="openChecklistDetail('${esc(String(p.linked.id))}')">Open Full Checklist</button></div></div>`
    :p.items.length
      ?`<div class="detail-card maintenance-checklist-card"><div class="maintenance-checklist-head"><div><div class="maintenance-eyebrow">WORK CHECKLIST</div><h3>Procedure Checklist</h3><div class="muted tiny">Source-backed high-level work list</div></div><div class="maintenance-progress-count"><b>${p.done}</b><span>/ ${p.items.length}</span><small>${p.pct}% complete</small></div></div><div class="maintenance-progress-track"><i style="width:${p.pct}%"></i></div><div class="maintenance-procedure-list">${p.items.map((i,n)=>`<div class="maintenance-procedure-item ${i.done?'done':''}"><label class="maintenance-step-main"><input type="checkbox" ${i.done?'checked':''} onchange="toggleMaintenanceProcedureItem(${m.id},'${esc(String(i.id))}',this.checked)"><span class="maintenance-step-number">${i.done?'✓':n+1}</span><span class="maintenance-step-copy"><b>${esc(i.text)}</b>${i.note?`<small>${esc(i.note)}</small>`:''}</span></label><button class="maintenance-note-btn ${i.note?'has-note':''}" onclick="openMaintenanceProcedureNote(${m.id},'${esc(String(i.id))}')">${i.note?'Edit note':'Add note'}</button></div>`).join('')}</div>${p.done?`<div class="maintenance-checklist-actions"><button class="secondary" onclick="resetMaintenanceProcedure(${m.id})">Reset Checklist</button></div>`:''}</div>`
      :`<div class="detail-card maintenance-checklist-card"><div class="maintenance-checklist-head"><div><div class="maintenance-eyebrow">WORK CHECKLIST</div><h3>Procedure Checklist</h3></div></div><div class="empty">No procedure checklist is attached to this maintenance item yet.</div></div>`;
  const relatedWork=maintenanceRelatedWorkHTML(m);
...
async function renderSystem(){
  ...
  page.innerHTML=`<div class="grid"><div class="card span-5"><h1>System & Sync</h1><div class="kv"><span>Account role</span><b>${esc(cloudRole||'—')}</b></div><div class="kv"><span>Connection</span><b>${navigator.onLine?'Online':'Offline'}</b></div><div class="kv"><span>Last synced</span><b>${typeof lastCloudSyncAt!=='undefined'&&lastCloudSyncAt?lastCloudSyncAt.toLocaleTimeString():'—'}</b></div><div class="kv"><span>App version</span><b>v${APP_VERSION}</b></div><div class="action-row" style="margin-top:12px"><button class="primary" onclick="forceCloudReload()">Reload Shared Data</button><button class="secondary" onclick="exportCoreData()">Export JSON</button><button class="secondary" data-tracker-update-button onclick="forceLatestAppVersion()">Force Latest Version</button><a class="btn secondary" href="https://mtm951.github.io/N594ZS-Tracker/?forceUpdate=${Date.now()}" target="_blank" rel="noopener noreferrer" title="Open newest published version in a separate tab">Fresh version ↗</a><div class="tiny muted" role="status" aria-live="polite" data-tracker-update-status style="width:100%">Make sure your cloud status says Synced before updating.</div><button class="secondary" id="installAppBtn" onclick="installN594ZSApp()" style="display:none">Install App</button></div></div>
